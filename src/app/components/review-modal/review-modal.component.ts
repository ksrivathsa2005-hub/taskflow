            ...
            <div class="bg-white rounded-[2rem] w-full max-w-md overflow-hidden shadow-2xl">
                <div class="px-8 py-6 border-b border-slate-100 flex justify-between items-center">
                    ...
                    <button (click)="closeModal()" type="button"
                        ...
                        class="bg-slate-50 p-2 rounded-xl text-slate-400 hover:text-slate-600 transition-colors focus-visible:ring-2 focus-visible:ring-indigo-500 outline-none">
                ...
                <form (ngSubmit)="submitReview()" class="p-8 space-y-6">
                    <!-- Star Rating -->
                    <div>
                        <label id="rating-label" class="block text-sm font-bold text-slate-700 mb-3">Rating <span class="text-red-500" aria-hidden="true">*</span></label>
                        <div class="flex gap-2" role="radiogroup" aria-labelledby="rating-label">
                            <button *ngFor="let star of [1,2,3,4,5]"
                                ...
                                class="transition-all hover:scale-125 active:scale-95 focus:outline-none group">
                                <lucide-icon [img]="Star" 
                                    [class.fill-amber-400]="star <= rating" 
                                    [class.text-amber-400]="star <= rating" 
                                    [class.text-slate-300]="star > rating" 
                                    class="w-8 h-8 pointer-events-none group-focus-visible:ring-2 group-focus-visible:ring-indigo-500 rounded-full"></lucide-icon>
                            </button>
                        </div>
                        <p *ngIf="rating > 0" class="text-xs text-indigo-600 mt-2 font-medium" aria-live="polite">Rating: {{rating}} out of 5</p>
                        <p *ngIf="showRatingError" class="text-xs text-red-500 mt-2 font-medium" role="alert">Please select a rating</p>
                    </div>

                    <!-- Review Text -->
                    <div>
                        <label for="review-comment" class="block text-sm font-bold text-slate-700 mb-2">Your Review <span class="text-red-500" aria-hidden="true">*</span></label>
                        <textarea [(ngModel)]="comment" name="comment" id="review-comment" rows="4" placeholder="Share your experience..."
                            class="w-full px-4 py-3 border border-slate-100 rounded-xl outline-none focus:ring-2 focus:ring-indigo-500 resize-none transition-shadow"
                            [class.border-red-300]="showCommentError"></textarea>
                        <p *ngIf="showCommentError" class="text-xs text-red-500 mt-2 font-medium" role="alert">Please write a review</p>
                    </div>
