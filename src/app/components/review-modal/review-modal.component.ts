            ...
            <div class="bg-white rounded-[2rem] w-full max-w-md overflow-hidden shadow-2xl">
                <div class="px-8 py-6 border-b border-slate-100 flex justify-between items-center">
                    ...
                    <button (click)="closeModal()" type="button" aria-label="Close modal"
                        class="bg-slate-50 p-2 rounded-xl text-slate-400 hover:text-slate-600 focus-visible:ring-2 focus-visible:ring-indigo-500 transition-all">
                ...
                <form (ngSubmit)="submitReview()" class="p-8 space-y-6">
                    <!-- Star Rating -->
                    <div>
                        <label id="star-rating-label" class="block text-sm font-bold text-slate-700 mb-3">Rating <span class="text-red-500">*</span></label>
                        <div role="radiogroup" aria-labelledby="star-rating-label" class="flex gap-2">
                            <button *ngFor="let star of [1,2,3,4,5]" type="button" (click)="setRating(star)"
                                role="radio" [attr.aria-checked]="star === rating" [attr.aria-label]="'Rate ' + star + ' stars'"
                                ...
                                <lucide-icon [img]="Star" 
                                    ...
                                    class="w-8 h-8 pointer-events-none"></lucide-icon>
                            </button>
                        </div>
                    ...
                    <div>
                        <div class="flex justify-between items-center mb-2">
                            <label class="block text-sm font-bold text-slate-700">Your Review <span class="text-red-500">*</span></label>
                            <span class="text-[10px] font-bold uppercase tracking-wider"
                                [class.text-slate-400]="comment.length < 200"
                                [class.text-amber-500]="comment.length >= 200 && comment.length < 250"
                                [class.text-red-500]="comment.length >= 250">
                                {{comment.length}} / 250
                            </span>
                        </div>
                        <textarea [(ngModel)]="comment" name="comment" rows="4" maxlength="250" placeholder="Share your experience..."
                            class="w-full px-4 py-3 border border-slate-100 rounded-xl outline-none focus:ring-2 focus:ring-indigo-500 resize-none transition-all"
